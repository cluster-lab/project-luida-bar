...
Please also complete the following steps in advance:
...
1. [Register Experiment Information on Web Console](#register-experiment-information-on-web-console)
2. [Initial Setup in Unity](#initial-setup-in-unity)
3. [Setup Experiment Variables & Trials Count](#setup-experiment-variables--trials-count)
4. [Adjust State Transitions](#adjust-state-transitions)
5. [Confirm Prefabs for the Tutorial](#confirm-prefabs-for-the-tutorial)
6. [Add Objects for Each State](#add-objects-for-each-state)
7. [Implement Trials Session](#implement-trials-session)
8. [Implement Practice Session](#implement-practice-session)
9. [Adding Data Recording and Upload Objects](#adding-data-recording-and-upload-objects)
10. [Linking the Questionnaire](#linking-the-questionnaire)
11. [Preparation Before Uploading](#preparation-before-uploading)
12. [Upload to cluster](#upload-to-cluster)
13. [Register World ID and Avatar on Web Console](#register-world-id-and-avatar-on-web-console)
14. [Wait for Publication on LUIDA](#wait-for-publication-on-luida)

---

## Register Experiment Information on Web Console

1. Open the web console: [LUIDA Web Console](https://cluster-lab.github.io/project-luida-web-console/) (If needed, create a GitHub account to access the console).
...
3. Register the experiment recruitment information:
   ...
   <table>
     <tr>
       <td><img src="https://github.com/user-attachments/assets/3ccc2bb5-cb02-495e-b876-da06d21b4b14" alt="Register Experiment Step 1" width="500"/></td>
       <td><img src="https://github.com/user-attachments/assets/f75c5f36-3030-4f72-bbf4-1259d42dac09" alt="Register Experiment Step 2" width="500"/></td>
     </tr>
   </table>

4. Press **Register** to save the information. Note the generated experiment identifier (eID).

   ![eID Confirmation](https://github.com/user-attachments/assets/758a26d4-0fda-47d4-8528-cb98f3f2f2c7)

5. Register the questionnaire to be used in the experiment:
   1. Go to the questionnaire list screen by clicking **Questionnaires**.
   2. Click **Register New Questionnaire** button to move to the registration screen for the questionnaire to be used in the experiment.
...
Launch the downloaded implementation template from Unity Hub, and continue the following steps.
...
To realize this transition, edit the transition in the editor window:
...
2. Edit `TaskManager.js` as follows.
   ...
   $.state.handOffset = new Quaternion().setFromEulerAngles(
     new Vector3(0, 90, 0)
   ); // Set the offset to correct the rotation difference between the physical hand (controller) and the virtual hand
   ```
   ...
   if (!$.state.isTouchable) {
     // If the green ball was just touched and should not respond for a while
     $.state.timer = $.state.timer + 1; // Increment timer by 1

     if ($.state.timer > 10) {
       // If 10 frames have passed since the green ball was touched
     ...
     // If the green ball has been detected as touched (when it becomes true)
     /*
...
## Register World ID and Avatar on Web Console
