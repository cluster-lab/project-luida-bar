...
- **Instruction**: NextStateButton × 1 (Change text to `Practice`), Message × 1 (Change text to an instruction of the practice session)
    ...
    ```text
    From now on, you will perform a task of touching the green sphere with your right index finger and answering questions. 
    Let's practice a few times first. When you're ready, hide the controller in the settings screen, look forward, and press the "Practice" button.
    ```
...
- **Preparation**: NextStateButton × 1 (Change text to `Start`), Message × 1 (Change text to an instruction of the trial session)
    ...
    ```text
    The practice (3 times) is now complete. 
    From here on, this is the main event. Please perform the task 22 times following the same procedure. 
    When you're ready, look forward and press the start button.
    ```
...
- **End**
  - Message × 1 (Change text to an instruction for leaving the experiment)
    ...
    The reward cluster points will be granted at a later date. 
    Please dive through the gate in front of you to exit.
    ```
    
    </details>
    
  - World gate prefab (`Assets/ClusterGAMEWORLDCENTER/Prefabs/WorldGateToClusterLobby.prefab`) × 1. After adding:
    - Set the position to (0, 0, 1.5)
    - Delete its child gameObject `SignBoard`
    - On its child gameObject `WorldGate`, set the value of the field `World Or Event Id` to `006d765e-f961-435b-a183-77c35a42e241` (World ID of the LUIDA recruitment world)
...
2. Edit `TaskManager.js` as follows:
    ...
    $.state.isTouchable = true; // Flag to allow touching the green sphere
    $.state.handOffset = new Quaternion().setFromEulerAngles(new Vector3(0, 90, 0)); // Set offset to correct rotation difference between the physical hand (controller) and virtual hand
    ```
    ...
    ```javascript
    // Initialize variable values: player, origin, target
    ...
    $.state.gain = 1; // Keep the gain value as 1 before touching the origin
    ...
    // Calculate the virtual hand's position: relative position from the origin of the physical hand (controller) × gain
    ...
    // Sync the rotation of the virtual hand with the physical hand
    ...
    if (!$.state.isTouchable) { // If the green sphere was just touched, it should not react for a while
       $.state.timer = $.state.timer + 1; // Increase the timer by 1
      
       if ($.state.timer > 10) { // After 10 frames since the green sphere was touched
           $.state.isTouchable = true; // Allow touching the green sphere again from the next frame
           $.state.timer = 0; // Reset the timer
       } else {
           $.setStateCompat("this", "isSphereTouched", false); // Keep the flag to detect if the green sphere was touched as false
       }
    } else if ($.getStateCompat("this", "isSphereTouched", "boolean")) { // When the green sphere is detected as touched (turns true)
       /*
           When the green sphere is touched, the value of $.getStateCompat("this", "isSphereTouched", "boolean") should change,
           Please add the CCK component `On Collide Item Trigger` to the item with this script,
           and set it to broadcast a message with key `isSphereTouched` and value=true directed towards this item.
       */
    
       $.state.isTouchable = false; // Set the flag to prevent double clicks to false
       $.setStateCompat("this", "isSphereTouched", false); // Reset the flag that detects if the green sphere was touched
    
       if ($.state.isReaching) {
           // If the green sphere at the target location is touched
           onTargetTouched();
       } else {
           // If the green sphere at the origin is touched
           onOriginTouched();
       }
    }
    ```
    ...
    ```javascript
    // Executed when the green sphere at the origin is touched
    function onOriginTouched () {
       $.subNode("Sphere").setPosition($.state.targetPos); // Move the green sphere to the target location
       $.state.gain = $.state.currentCondition["gain"] ? parseFloat($.state.currentCondition["gain"]) : 1; // Set the gain value for this trial
       $.state.isReaching = true; // Set the reaching (stretching hand to the target location) flag to true
    }
    
    // Executed when the green sphere at the target location is touched
    function onTargetTouched () {
       $.sendSignalCompat("this", "state_triggerTransition"); // Transition to the next state (question answering phase)
       /*
           Please add the CCK component `Global Logic` to the item with this script,
           In that `Global Logic`, detect the key `state_triggerTransition` directed towards this item,
           and broadcast a signal with key `state_triggerTransition` globally.
       */
    
       $.state.isReaching = false; // Set the reaching (stretching hand to the target location) flag to false
    }
    ```
    
    </details>
3. Add CCK components to the `TaskManager` gameobject: Add the CCK components to the `TaskManager` gameobject according to the diagram, and set the values. Below are the reasons for adding each component and how they function when set according to the diagram:
    - **Global Logic**
        - Purpose: To transition to the next state when the sphere is touched after moving.
        - Actual Function: It receives a signal with key `state_triggerTransition` from itself (e.g., the first line of the onTargetTouched function in TaskManager.js) and broadcasts a signal with key `state_triggerTransition` globally.
    - **On Collide Item Trigger**
        - Purpose: To detect contact with the sphere.
        - Actual Function: When colliding with other objects, it sets `isSphereTouched` to true directed towards itself, which is then processed in TaskManager.js > tick function using `if ($.getStateCompat("this", "isSphereTouched", "boolean"))`.
...
![add-gameobject-to-state-no-prefab-no-script](https://github.com/user-attachments/assets/2896fba5-99d2-4609-bfb7-b59802a1266c)
    ...
    3. Add the virtual hand prefab to the scene as a child gameobject of `TaskManager`.
