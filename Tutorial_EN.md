...
4.  **Create questionnaires**: Scroll down and perform the following operations in the questionnaire registration form:
    ...
    2. Post-experiment questionnaire
        1. Click "Add Questionnaire" button -> Select "IPQ" from "Template Selection" -> Click "Add" button
        2. Click the "Question List" button for the added questionnaire and verify the added questions.
...
3. Click the `Add Listener` button at the cells where the `Text_RedFont` and `Text_BlueFont` columns intersect with the `Trial - Start` row, and configure as shown in the figure below.
    - _Configuration explanation_
        - _Only visible during `Trial - Start` state_
        - _If the trial's condition `font`=`R`, only the **red font** is shown; if `font`=`B`, only the **blue font** is shown_
        - _If the trial's condition `text`=`Red`, both texts show **Red**; if `text`=`Blue`, both show **Blue**_
        - _If the trial's condition `depth`=`near`, both are positioned at **z=1**; if `depth`=`far`, both at **z=3**_
...
5. **Configure to send answer and trigger state transition on click**: Find the Answer_Red and Answer_Blue objects themselves, add `Interact Item Trigger` and `Luida To Next State Gimmick` components to each, and configure as shown in the figure below.
    - See CCK documentation for the relationship between Triggers and Gimmicks: https://docs.cluster.mu/creatorkit/world/trigger-gimmick/
        - Expected behavior (for Answer_Red):
            1. When clicked, `Interact Item Trigger` activates, sending a message with `isRed=true` to `LUIDA-DataCollector` (data recording object, details later), then sending a `toNextState` message to This (itself).
                -  If the `LUIDA-DataCollector` game object doesn't exist, right-click in Hierarchy and select `LUIDA > Data Collector` to create it: <img width="471" height="752" alt="image" src="https://github.com/user-attachments/assets/66a310c7-6cc6-49df-90fa-875c4820b22d" />
            2. When the `toNextState` message arrives at This (itself), `Luida To Next State Gimmick` activates and triggers state transition.
...
```javascript
function getRandomInt(max) { // Define function to generate random integer
...
2. Copy and paste the following script and save the file:
```JavaScript
return {
  d: CONDITION['depth'], // depth condition for this trial (value of between-subject variable depth)
  req: CONDITION['request'], // request condition for this trial (value of within-subject variable request)
  font: CONDITION['font'], // font condition for this trial (value of within-subject variable font)
  text: CONDITION['text'], // text condition for this trial (value of within-subject variable text)
